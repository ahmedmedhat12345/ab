...
def get_explanation(features_dict, predicted_price, cluster_label, market_stats, tokenizer, model):
    ...
    # 2. Construct Prompt (STRICT FORMAT - LATEST)
    prompt_text = f"""
IMPORTANT:

Completely replace all previous explanation logic in the project.
Do not use OpenAI or old prompts.
Do not hardcode reasoning.
Use google/flan-t5-base (instruction-tuned Transformer) exactly like in the lecture notebook: concise, context-aware, instruction-following.

TASK:

You are generating a short, professional, and clear explanation of a real estate regression prediction.  
Your explanation must satisfy the project requirement:

1. Explain **why this price was predicted** using property features.  
2. Check **market realism** against dataset statistics.  
3. Output must be **short, simple, readable**, suitable for Streamlit display.
...
<1–2 sentence explanation referencing key features>

Market Reality Check:
<1 sentence, assess realism: Realistic | Underpriced | Overpriced | Possibly Erroneous>
...
- Use feature-based causal reasoning: e.g., “Because the house has 3 bedrooms and grade 10…”  
- Include numeric price-per-sqft comparison with median/avg  
- Flag deviations >40% as “Possibly Erroneous”  
- No filler, no emojis, no assistant tone  
- Keep it concise for Streamlit display
"""
