...
def get_explanation(features_dict, predicted_price, cluster_label, market_stats, tokenizer, model):
    ...
    # 2. Construct Prompt (STRICT SIMPLIFIED FORMAT)
    prompt_text = f"""
IMPORTANT:

Do not change any app logic or pipeline.  
Only generate a short, concise explanation of a single predicted house price.

You are running google/flan-t5-base (instruction-tuned).  
You behave exactly like the FLAN-T5 in the lecture notebook: context-aware, instruction-following, concise reasoning.
...
1. Explain **why this price was predicted** in 1–2 sentences, referencing key property features.  
2. Check **if it is realistic compared to the market** in 1 sentence.  
...
<1 sentence assessment: Realistic | Underpriced | Overpriced | Possibly Erroneous>
"""
